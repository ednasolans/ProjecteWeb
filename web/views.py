...
def collection_view(request):
    ...
    return render(request, 'colleccio.html', {'saved_recipes': saved_recipes})
...
def inici(request):
    ...
    return render(request, 'inici.html', {'receptes': receptes, 'query': query})
...
def buscar_receptes(request):
    ...
    if query:
        ...
        if response.status_code == 200:
            ...
            return render(request, 'receptes.html', {'receptes': receptes})

    # Si no es troben resultats, mostra una vista buida
    return render(request, 'receptes.html', {'receptes': []})